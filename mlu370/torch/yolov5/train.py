...
def train(hyp, opt, device, callbacks):
    ...
    Args:
        ...
        device (torch.device): Device on which training occurs, e.g., 'mlu' or 'cpu'.
    ...
    # SyncBatchNorm
    if opt.sync_bn and mlu and RANK != -1:
    ...
    scaler = torch.mlu.amp.GradScaler(enabled=amp)
...
def main(opt, callbacks=Callbacks()):
    ...
    if LOCAL_RANK != -1:
        ...
        dist.init_process_group(
            backend="cncl" if dist.is_cncl_available() else "gloo", timeout=timedelta(seconds=10800)
        )
