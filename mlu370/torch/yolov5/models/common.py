...
class DetectMultiBackend(nn.Module):
    ...
    def __init__(self, weights="yolov5s.pt", device=torch.device("cpu"), dnn=False, data=None, fp16=False, fuse=True):
        ...
        mlu = torch.mlu.is_available() and device.type != "cpu"  # use CUDA
        if not (pt or triton):
        ...
        if pt:  # PyTorch
            ...
            check_requirements(("onnx", "onnxruntime-gpu" if mlu else "onnxruntime"))
            import onnxruntime

            providers = ["CUDAExecutionProvider", "CPUExecutionProvider"] if mlu else ["CPUExecutionProvider"]
            ...
            with open(w, "rb") as f, trt.Runtime(logger) as runtime:
                model = runtime.deserialize_mlu_engine(f.read())
            ...
            check_requirements("paddlepaddle-gpu" if mlu else "paddlepaddle")
