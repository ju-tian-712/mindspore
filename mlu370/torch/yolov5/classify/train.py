...
def train(opt, device):
    ...
    best_fitness = 0.0
    scaler = amp.GradScaler(enabled=mlu)
    ...
    for epoch in range(epochs):  # loop over the dataset multiple times
        ...
        for i, (images, labels) in pbar:  # progress bar
            ...
            # Forward
            with amp.autocast(enabled=mlu):  # stability issues when enabled
            ...
            if RANK in {-1, 0}:
                ...
                mem = "%.3gG" % (torch.mlu.memory_reserved() / 1e9 if torch.mlu.is_available() else 0)  # (GB)
...
def main(opt):
    ...
    if LOCAL_RANK != -1:
        ...
        assert torch.mlu.device_count() > LOCAL_RANK, "insufficient CUDA devices for DDP command"
        torch.mlu.set_device(LOCAL_RANK)
        device = torch.device("mlu", LOCAL_RANK)
        dist.init_process_group(backend="cncl" if dist.is_cncl_available() else "gloo")
