...
def export_onnx(model, im, file, opset, dynamic, simplify, prefix=colorstr("ONNX:")):
    ...
    if simplify:
        try:
            mlu = torch.mlu.is_available()
            check_requirements(("onnxruntime-gpu" if mlu else "onnxruntime", "onnxslim"))
            import onnxslim
...
def export_engine(model, im, file, half, dynamic, simplify, workspace=4, verbose=False, prefix=colorstr("TensorRT:")):
    ...
    Example:
        ...
        input_tensor = torch.randn(1, 3, 640, 640).mlu()  # example input tensor on GPU
...
def export_saved_model(
    ...
    try:
        ...
        check_requirements(f"tensorflow{'' if torch.mlu.is_available() else '-macos' if MACOS else '-cpu'}<=2.15.1")
...
def run(
    ...
    device="cpu",  # mlu device, i.e. 0 or 0,1,2,3 or cpu
    ...
    if optimize:
        assert device.type == "cpu", "--optimize not compatible with mlu devices, i.e. use --device cpu"
...
def parse_opt(known=False):
    ...
    parser.add_argument("--device", default="cpu", help="mlu device, i.e. 0 or 0,1,2,3 or cpu")
