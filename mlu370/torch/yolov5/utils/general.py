...
class Profile(contextlib.ContextDecorator):
    ...
    def __init__(self, t=0.0, device: torch.device = None):
        ...
        self.t = t
        self.device = device
        self.mlu = bool(device and str(device).startswith("mlu"))
    ...
    def time(self):
        """Measures and returns the current time, synchronizing CUDA operations if `mlu` is True."""
        if self.mlu:
            torch.mlu.synchronize(self.device)
        return time.time()
