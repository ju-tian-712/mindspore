...
def export_models(ckpt_path):
    ...
    input_arr = Tensor(np.zeros([1, 12, config.testing_shape[0] // 2, config.testing_shape[1] // 2]), mindspore.float32)
...
def run_train():
    ...
    for epoch_idx in range(config.max_epoch):
        for step_idx, data in enumerate(data_loader):
            ...
            input_shape = mindspore.Tensor(tuple(input_shape[::-1]), mindspore.float32)
