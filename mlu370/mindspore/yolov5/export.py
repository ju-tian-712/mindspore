...
def run_export():
    ...
    if config.device_target == "Ascend":
        mindspore.set_context(device_id=config.device_id)
    ...
    param_dict = mindspore.load_checkpoint(config.ckpt_file)
    mindspore.load_param_into_net(network, param_dict)

    input_data = mindspore.numpy.zeros([config.batch_size, config.testing_shape[0], config.testing_shape[1], 3], mindspore.int8)

    mindspore.export(network, input_data, file_name=config.file_name, file_format=config.file_format)
