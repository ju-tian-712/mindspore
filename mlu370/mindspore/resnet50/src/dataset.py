...
# def create_dataset1(dataset_path, do_train, repeat_num=1, batch_size=32, target="Ascend", subset_ratio=0.5):
#     """
...
#         subset_ratio(float): the ratio of subset to load. Default: 0.5
...
#     # Define the subset size to load
...
#     # Modify the dataset to return only one data item each time
#     ds = ds.map(operations=lambda x, y: (x[0], y[0]), input_columns=["image", "label"], num_parallel_workers=8)

#     return ds

def create_dataset1(dataset_path, do_train, repeat_num=1, batch_size=32, target="Ascend", subset_ratio=0.004):
    """
    create a train or evaluate cifar10 dataset for resnet50
    Args:
        dataset_path(string): the path of dataset.
